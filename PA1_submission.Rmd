...
average (by median) number of steps for that time interval over all days.
...
```{r imputeSteps, echo=TRUE}
    ...
    for(i in 1:nrow(impData)){
        if(is.na(impData[i,]$steps)) {
            impData[i,]$steps <- impData[i,]$medSteps
        }
    }
    impData <- select(impData, -medSteps)
```
...
```{r dayOfWeek}
    ...
    dowDFts <- ggplot(dowDF, aes(x=interval, y=medSteps)) + geom_line() +
        labs(x="Interval", y="Median number of steps",
             title="Average Daily Activity")
    facetPlot <- dowDFts + facet_grid(day ~ .)
    print(facetPlot)
```

From the plots it is clear that there is much more activity during the middle of
the day on the weekends than on the weekdays. This makes sense if the subject
works at an office job during the weekdays.
